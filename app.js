...
// ---- V2.0 Podcast player (único)
function initPodcastV2(){
  const audio = $("podcastPlayer");
  if(!audio) return;

  const playBtn = $("podcastPlayBtn");
  const pauseBtn = $("podcastPauseBtn");
  const restartBtn = $("podcastRestartBtn");
  const speedBtn = $("podcastSpeedBtn");
  const timeLbl = $("podcastTime");

  const keyPos = "isivolt_podcast_pos";
  const keyRate = "isivolt_podcast_rate";
  const savedRate = Number(localStorage.getItem(keyRate) || "1");
  if(savedRate && !Number.isNaN(savedRate)) audio.playbackRate = savedRate;

  audio.addEventListener("loadedmetadata", ()=>{
    const savedPos = Number(localStorage.getItem(keyPos) || "0");
    if(savedPos && !Number.isNaN(savedPos) && savedPos < (audio.duration||0)-1){
  ...
  audio.addEventListener("timeupdate", ()=>{
    ...
    if(Math.floor(audio.currentTime) % 5 === 0){
      localStorage.setItem(keyPos, String(audio.currentTime));
    }
  });
  audio.addEventListener("pause", ()=> localStorage.setItem(keyPos, String(audio.currentTime)));
  window.addEventListener("beforeunload", ()=> localStorage.setItem(keyPos, String(audio.currentTime)));

  playBtn?.addEventListener("click", async ()=>{
    try{ await audio.play(); }
    catch(e){ toast("No se pudo reproducir. Asegura que el .m4a está en /podcast/ del repo.", "warn", "Podcast"); }
  });
  pauseBtn?.addEventListener("click", ()=> audio.pause());
  restartBtn?.addEventListener("click", ()=>{ audio.currentTime = 0; audio.play().catch(()=>{}); });

  const rates=[1,1.25,1.5,1.75,2];
  const refresh=()=>{ if(speedBtn) speedBtn.textContent = `${audio.playbackRate}×`; };
  refresh();
  speedBtn?.addEventListener("click", ()=>{
    const idx = rates.indexOf(audio.playbackRate);
    const next = rates[(idx+1)%rates.length];
    audio.playbackRate = next;
    localStorage.setItem(keyRate, String(next));
    refresh();
  });
}
...
function updateTimerFill(){
  ...
  const left = (state?.timer?.remainingSeconds ?? total);
  if(!total){ bar.style.width="0%"; return; }
  const done = Math.max(0, Math.min(1, (total-left)/total));
  bar.style.width = `${Math.round(done*100)}%`;
}


// ---- V2.0 Informe mensual (texto/CSV) -> checklist agrupado por Planta/MacroZona
function extractFirstCodeFromLine(line){
  const t = String(line||"").trim();
  if(!t) return null;
  const m = t.match(/(S[A-Z]\d{3}[A-Z]?|S[1-9]\d{3}[A-Z]?|[0-9][A-Z]\d{3}[A-Z]?|[0-9][0-9]\d{3}[A-Z]?)/i);
  return m ? m[1] : null;
}
...
function monthlyRender(){
  const host = $("monthlyList");
  if(!host) return;
  const items = monthlyLoad();
  const groups = new Map();
  for(const it of items){
    const key = `${it.planta}|${it.macroZona}`;
    if(!groups.has(key)) groups.set(key, {planta: it.planta, macroZona: it.macroZona, items: []});
    groups.get(key).items.push(it);
  }
  const sorted = Array.from(groups.values()).sort((a,b)=>{
    if(a.planta===b.planta) return String(a.macroZona).localeCompare(String(b.macroZona));
  ...
  for(const g of sorted){
    ...
    for(const it of g.items){
      ...
      row.className="monthly-item" + (it.done ? " done" : "");
      row.innerHTML = `
        <input type="checkbox" ${it.done?"checked":""} />
        <div>
          <div><b>${escHtml(it.codigoHospital)}</b> <span class="muted tiny">${escHtml(it.tipo||"")}</span></div>
          <div class="meta muted">${escHtml(it.descripcion||"")}</div>
        </div>`;
      const cb=row.querySelector("input");
      cb.addEventListener("change", ()=>{
        const all=monthlyLoad();
        const idx=all.findIndex(x=>x.id===it.id);
        if(idx>=0){
          all[idx].done = cb.checked;
          all[idx].doneAt = cb.checked ? new Date().toISOString() : "";
          monthlySave(all);
          monthlyRender();
        }
      });
...
function monthlyBuildFromLines(lines){
  ...
  for(let i=0;i<lines.length;i++){
    ...
    if(!line) continue;
    const codeRaw=extractFirstCodeFromLine(line);
    if(!codeRaw) continue;
    const parsed=parseHospitalCode(codeRaw);
    ...
    const desc=line.replace(codeRaw,"").trim();

    out.push({
      id:`m_${now}_${i}`,
      codigoHospital:d.codigoHospital,
      planta:d.planta,
...
function initMonthlyV2(){
  const ta=$("monthlyImportText");
  const file=$("monthlyImportFile");
  const buildBtn=$("monthlyBuildBtn");
  const clearBtn=$("monthlyClearBtn");

  buildBtn?.addEventListener("click", ()=>{
    const lines=String(ta?.value||"").split(/\r?\n/);
    const newItems=monthlyBuildFromLines(lines);
    const existing=monthlyLoad();
    const set=new Set(existing.map(x=>x.codigoHospital));
    for(const it of newItems){
      if(!set.has(it.codigoHospital)){ existing.push(it); set.add(it.codigoHospital); }
    }
    monthlySave(existing);
    monthlyRender();
    toast("Checklist mensual generado.", "ok", "Mensual");
  });

  clearBtn?.addEventListener("click", ()=>{
    if(confirm("¿Vaciar checklist mensual?")){
      monthlySave([]); monthlyRender();
    }
  });
